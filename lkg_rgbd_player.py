...
void main() {
    ...
    for (int i = 0; i < 3; i++) {
        float x = TexCoord.x + float(i) * subp;
        float y = TexCoord.y;
        
        // Calculate view index (normalized 0.0 to 1.0)
        float view = (x + y * slope) * pitch - center;
...
class LKGPlayer:
    def __init__(self, args):
        ...
        # Calibration defaults for LKG Go (Original values)
        self.pitch = 47.5
        self.slope = -5.5
        self.center = 0.0
        self.subp = 1.0 / (1440.0 * 3.0)
    ...
    def load_calibration(self):
        ...
        # Auto-search for calibration file
        if not calib_file:
        ...
        if calib_file and os.path.exists(calib_file):
            try:
                with open(calib_file, 'r') as f:
                    ...
                    screen_w = config.get('screenW', {}).get('value', 1440.0)
                    self.subp = 1.0 / (screen_w * 3.0)
                    print(f"Loaded calibration from {calib_file}")
                    print(f"  pitch={self.pitch}, slope={self.slope}, center={self.center}, invView={self.invView}")
            except Exception as e:
                print(f"Warning: Failed to load calibration: {e}")
        else:
            print("No calibration file found, using defaults")
            print(f"  pitch={self.pitch}, slope={self.slope}, center={self.center}")
    ...
    def start_video(self, path):
        ...
        if self.args.with_audio:
            ...
            subprocess.Popen(audio_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def run(self):
        ...
        # Frame buffer info
        if self.args.input.endswith(('.jpg', '.png')):
            ...
            frame_w, frame_h = self.get_video_info(self.args.input)
        ...
        while not glfw.window_should_close(self.window) and self.running:
            ...
            # Update texture
            GL.glBindTexture(GL.GL_TEXTURE_2D, self.texture)
            GL.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGB, frame_w, frame_h, 0, GL.GL_RGB, GL.GL_UNSIGNED_BYTE, raw_frame)
            
            # Set uniforms
            GL.glUniform1f(GL.glGetUniformLocation(self.shader_program, "pitch"), self.pitch)
            GL.glUniform1f(GL.glGetUniformLocation(self.shader_program, "slope"), self.slope)
